...
# low: -0.287713, 0.2887464
# high: 0.1728395, 0.5085018
# FDR-inter: 0.06207745
...
# view internal node
pdf("/n/holylfs05/LABS/dwang_lab/Lab/T2D_multiomics/2.analysis/anpan/5.callout/aa.pdf",
...
# check clade
clade2 <- tree_subset(treefile2, node=1419, levels_back=100)
ggtree(clade2, aes(color=group)) + # geom_tiplab() + 
  xlim(0, 9) + scale_color_manual(values=c("black", "red")) +
  layout_dendrogram()

# group clade based on node number
zz11=groupClade(as_tibble(treefile), 1344) # 1
zz22=groupClade(as_tibble(treefile), 1347) # 1
zz33=groupClade(as_tibble(treefile), 1879) # 1
zz44=groupClade(as_tibble(treefile), 1419) # 1
...
ggplot(metafile, aes(x=group, y=s__Roseburia_hominis, color=group)) + 
  geom_boxplot(outlier.shape = NA, width = 0.3) +
  geom_jitter(alpha=0.2, size=0.3, width = 0.1)
...
genefile2 <- genefile2[, colSums(genefile2==1)>5 & colSums(genefile2==1)<(nrow(metafile2)-5)]

summary(lm(X266 ~ group, data=metafile1)) # 0.002
summary(lm(X266 ~ group, data=metafile2)) # 0.006
...
View(res1[res1$V1 %in% c("UniRef90_A0A174GTL6",
                         "UniRef90_G2SXA1",
                         "UniRef90_G2SXA2"), ])

res2$uniref <- sapply(res2$gene, function(x) sub(":.*", "", x))
res2 <- merge(go2uniref90, res2, by.x="V1", by.y="uniref", all.y=T)
View(res2[res2$V1 %in% c("UniRef90_A0A174GTL6",
                         "UniRef90_G2SXA1",
                         "UniRef90_G2SXA2"), ])

rm(go2uniref90, ko2uniref90)
save.image(file="/n/holylfs05/LABS/dwang_lab/Lab/T2D_multiomics/3.figures/fig6/figure6b2.RData")
...
tableS10 <- merge(res11[, c(4,6,7,9,11)],
                  res22[, c(4,6,7,9,11)],
                  by="gene", all = T)
write.xlsx(tableS10, file="/n/holylfs05/LABS/dwang_lab/Lab/T2D_multiomics/3.figures/tableS10.xlsx")



View(res11[res11$gene %in% c("UniRef90_A0A174GTL6: Cholesterol dehydrogenase",
                             "UniRef90_G2SXA2: L-carnitine dehydratase/bile acid-inducible protein F",
                             "UniRef90_G2SXA1: L-carnitine dehydratase/bile acid-inducible protein F"),])
View(res22[res22$gene %in% c("UniRef90_A0A174GTL6: Cholesterol dehydrogenase",
                             "UniRef90_G2SXA2: L-carnitine dehydratase/bile acid-inducible protein F",
                             "UniRef90_G2SXA1: L-carnitine dehydratase/bile acid-inducible protein F"),])
...
genes <- genefile0[, c("sample_id",
                       "UniRef90_A0A174GTL6: Cholesterol dehydrogenase",
                       "UniRef90_G2SXA2: L-carnitine dehydratase/bile acid-inducible protein F",
                       "UniRef90_G2SXA1: L-carnitine dehydratase/bile acid-inducible protein F"
)]
colnames(genes)[2:4] <- c("A0A174GTL6: Choles... genase", 
                          "G2SXA2: bile acid...protein F", 
                          "G2SXA1: bile acid...protein F")
genes[,2:4] <- map_df(genes[,2:4],as.character)

# Convert from wide to long format
genes <- genes %>%
  pivot_longer(
    cols = c("A0A174GTL6: Choles... genase", 
             "G2SXA2: bile acid...protein F", 
             "G2SXA1: bile acid...protein F"),  # Columns to pivot/melt
    names_to = "gene",                    # Name for the new key column
    values_to = "presence"              # Name for the new value column
  ) %>% mutate(gene=factor(gene, levels=c("A0A174GTL6: Choles... genase", 
                                          "G2SXA2: bile acid...protein F", 
                                          "G2SXA1: bile acid...protein F")))
...
pdf("/n/holylfs05/LABS/dwang_lab/Lab/T2D_multiomics/3.figures/fig6/figure6b2.pdf",
    width = 7, height = 10, onefile = F) # Open a new pdf file
ggtree(treefile2, size = 0.3,
       layout = "fan", open.angle = 10) + 
  xlim(0, NA) +
  geom_hilight(mapping=aes(subset = node %in% c(1344, 1347, 1879),
                           fill=group),
               # type = "gradient", gradient.direction = 'rt',
               alpha = 0.3) +
  scale_fill_manual(values=clade_col) +
  new_scale_fill() +
  ...
  geom_fruit(
    ...
    axis.params=list(
      ...
      hjust      = 1,
      vjust      = 0.5),
    offset = 0.035, show.legend = F, inherit.aes = F) +
  scale_fill_manual(values = study_col, name="Study") +
  # new_scale_fill() +
  # geom_fruit(
  #   geom = geom_tile,
  ...
  scale_fill_manual(values = gene_col, name="Gene presence") +
  layout_rectangular() +
  theme(
